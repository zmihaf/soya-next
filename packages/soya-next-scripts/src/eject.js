...
const ownDependencies = ["chalk", "cross-spawn", "inquirer"];
...
const updateDependencies = (appPackage, ownPackage) => {
  ...
  devDependencies
    ...
    .forEach(devDependency => {
      ...
      appPackage.devDependencies[devDependency] =
        ownPackage.dependencies[devDependency];
    });
    ...
    const files = ["next.config.js"].concat(
      dirs.reduce(
        (files, dir) =>
          files.concat(
            fs
              .readdirSync(path.join(ownDir, dir))
