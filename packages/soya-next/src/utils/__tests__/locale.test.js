...
describe('Locale Utils', () => {
  describe('ensurePath', () => {
    it('should ensure non-default locale url to have locale segment', () => {
      ...
      expect(
        ensurePath('/', { language: 'en', country: 'id' }, 'id-id')
      ).toBe('/en/');
    });
  ...
  describe('toPath', () => {
    ...
    it('should throw an error if locale is not an object', () => {
      ...
      ).toThrow('Expected locale to be an object with country and language properties.');
    });
  ...
  describe('trimPath', () => {
    ...
    it('should remove locale segment from non-default locale url', () => {
      expect(
        trimPath('/en/about', 'id-id', ['id-id', 'en-id'])
      ).toBe('/about');
    });

    it('should not remove anything from default locale url', () => {
      expect(
        trimPath('/', 'id-id', ['id-id', 'en-id'])
      ).toBe('/');

      expect(
        trimPath('/about', 'id-id', ['id-id', 'en-id'])
      ).toBe('/about');
    });
  });
});
